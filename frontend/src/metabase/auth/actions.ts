...
export const LOGIN_OKTA = "metabase/auth/LOGIN_OKTA";
export const loginOkta = createAsyncThunk(
  LOGIN_OKTA,
  async (
    { code, state, redirectUrl = "/" }: LoginOktaPayload,
    { dispatch, rejectWithValue },
  ) => {
    try {
      // Предполагаем, что у SessionApi есть метод для обработки логина через Okta
      await SessionApi.createWithOktaAuth({ code, state });
      await dispatch(refreshSession()).unwrap();
      trackLoginOkta(); // Функция для отслеживания входа через Okta, если она у вас есть
      dispatch(push(redirectUrl));
    } catch (error) {
      return rejectWithValue(error);
    }
...
// Определение типа для Payload
interface LoginOktaPayload {
  code: string;
  state: string;
  redirectUrl?: string;
}
