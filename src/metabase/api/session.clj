...
(api/defendpoint POST "/google_auth"
  ...
  {token ms/NonBlankString}
  (log/info "Starting Google Auth with token:" token)
  ;; Use do-google-auth to create a user with test data
  (let [user (google/do-google-auth request)]
    ...
    (let [{session-uuid :id, :as session} (create-session! :sso user (request.u/device-info request))]
      (log/info "Session created with UUID:" session-uuid "and session:" session)
      (let [response {:id (str session-uuid)}]
        (log/info "Response object created:" response)
        ;; Set session cookies for the test user
        (log/info "Setting session cookies")
        (let [updated-response (mw.session/set-session-cookies request
                                                              response
                                                              session
                                                              (t/zoned-date-time (t/zone-id "GMT")))]
          (log/info "Session cookies set. Final response:" updated-response)
          ;; Return some response or user data if needed
          updated-response)))))


(api/defendpoint POST "/okta_auth"
  "Login for testing purposes via Okta."
  [:as {{:keys [code state]} :body, :as request}]
  (log/info "Received Okta Auth code:" code "and state:" state)

  ;; Обмен кода на токен
  (log/info "Exchanging code for token...")
  (let [token-response (http/post "https://dev-91515140.okta.com/oauth2/default/v1/token"
                                  {:form-params {:grant_type    "authorization_code"
                                                 :code          code
                                                 :redirect_uri  "http://90.156.227.253:3000/auth/forgot_password/"
                                                 :client_id     "0oadjayev40xqxwQB5d7"
                                                 :client_secret "hN3NziMhLFPhqWl8HX3BErGUJW_S-AX7PlzWIsjA3YnnXepIOo4bQzD1prDKbdp-"}
                                   :as :json})
        token-data (:body token-response)
        access-token (:access_token token-data)]

    (when-not access-token
      (log/error "Failed to receive access token. Response:" token-response)
      (throw (ex-info "Failed to receive access token" {:status-code 400})))

    (log/info "Access token received: " access-token)


    ;; Получение информации о пользователе с использованием токена доступа
    (log/info "Fetching user information...")
    (let [user-info-response (http/get "https://dev-91515140.okta.com/oauth2/default/v1/userinfo"
                                       {:headers {"Authorization" (str "Bearer " access-token)}
                                        :as :json})
          user-data (:body user-info-response)
          ;; Преобразование ответа Okta в формат, ожидаемый функцией do-okta-auth
          okta-request {:body {:first_name (:given_name user-data)
                               :last_name (:family_name user-data)
                               :email (:email user-data)}}
          user (google/do-okta-auth okta-request) ;; Вызов функции do-okta-auth для создания пользователя
          ;; Создание сессии для пользователя
          {session-uuid :id, :as session} (create-session! :sso user (request.u/device-info request))]

      (log/info "User and session created: " user session-uuid)

      ;; Структура ответа и установка кук сессии
      (let [response {:id (str session-uuid)}
            updated-response (mw.session/set-session-cookies request
                                                            response
                                                            session
                                                            (t/zoned-date-time (t/zone-id "GMT")))]
        (log/info "Session cookies set. Final response:" updated-response)
