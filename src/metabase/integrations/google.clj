...
(defn do-google-auth
  "Call to create a new user with test data"
  [{{:keys [token body]} :body, :as _request}]
  (do
    (log/info "Creating a new user with test data.")
    ;; Создание пользователя с тестовыми данными
    (let [test-user-data {:first_name "Test4"
                          :last_name  "User4"
                          :email      "test4.user@example.com"}
          ;; Вызов функции для создания нового пользователя
          user (google-auth-fetch-or-create-user! (:first_name test-user-data) 
                                                  (:last_name test-user-data) 
                                                  (:email test-user-data))]
      user)))


(defn do-okta-auth
  "Call to create a new user with data from the request"
  [{{:keys [first_name last_name email]} :body, :as _request}]
  (do
    (log/info "Creating a new user from request data.")
    ;; Создание пользователя с данными из запроса
    (let [user-data {:first_name first_name
                     :last_name last_name
                     :email email}
          ;; Вызов функции для создания нового пользователя
          user (google-auth-fetch-or-create-user! first_name last_name email)]
      user)))
